...
const createInterview = async (userId, resumeId, settings = {}) => {
  ...
  if (resume.user.toString() !== userId.toString()) {
    throw new AppError('You are not authorized to use this resume.', 403)
  }
  ...
  const activeInterview = await interviewRepository.findActiveInterviewByResume(userId, resumeId)
  ...
  const aiResponse = await generateInterviewQuestions(resume.extractedText, {
    company: settings.company || 'General',
    role: settings.role || 'Software Engineer',
    difficulty: settings.difficulty || 'Medium',
    experience: settings.experience || 'Fresher',
    focus: settings.focus || 'Mixed',
  ...
  if (!aiResponse || !Array.isArray(aiResponse.questions) || aiResponse.questions.length === 0) {
    throw new AppError('AI failed to generate interview questions. Please try again.', 502)
  }

  const validQuestions = aiResponse.questions.filter(
    item => item && typeof item.question === 'string' && item.question.trim()
  )

  if (validQuestions.length === 0) {
    throw new AppError('AI returned invalid interview questions. Please try again.', 502)
  }
