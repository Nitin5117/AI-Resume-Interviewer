...
const evaluateInterview = async (userId, interviewId) => {
  const existingInterview = await interviewRepository.getOwnedInterview(userId, interviewId)
  ...
  if (existingInterview.status === 'evaluating') {
    throw new AppError('Interview evaluation is already in progress.', 409)
  }

  const unanswered = existingInterview.questions.find(
  ...
  const interview = await interviewRepository.startEvaluation(userId, interviewId)

  if (!interview) {
    throw new AppError('Interview evaluation is already in progress or completed.', 409)
  }
  ...
  try {
    ...
    interview.durationInSeconds = Math.floor((interview.completedAt - interview.startedAt) / 1000)
